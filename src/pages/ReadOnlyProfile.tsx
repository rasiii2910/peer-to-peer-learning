...
const sampleUsers: Record<string, any> = {
  u1: {
    name: 'John Doe',
    yearBranch: '4th Year – Computer Science',
    email: 'john@example.com',
    phone: '999-888-7777',
    bio: 'Interested in full-stack development',
    avatar: null,
    profiles: [{ id: 'p1', key: 'github', url: 'https://github.com/johndoe' }],
    skills: [
      { id: 's1', name: 'React', level: 'Advanced' },
      { id: 's2', name: 'Node.js', level: 'Intermediate' },
    ],
    feedback: [
      { id: 'f1', mentor: 'Prof. Rao', rating: 5, text: 'Very dedicated.', date: '2025-06-12' },
    ],
  },
  u2: {
    name: 'Priya Verma',
    yearBranch: '3rd Year – Computer Science',
    email: 'priya@example.com',
    phone: '',
    bio: 'Competitive programmer & mentor',
    avatar: null,
    profiles: [{ id: 'p2', key: 'leetcode', url: 'https://leetcode.com/priya' }],
    skills: [
      { id: 's3', name: 'Algorithms', level: 'Advanced' },
      { id: 's4', name: 'CP', level: 'Advanced' },
    ],
    feedback: [
      { id: 'f2', mentor: 'Dr. Singh', rating: 4, text: 'Great guidance on algorithms.', date: '2025-07-12' },
    ],
  },
  u3: {
    name: 'Amit Sharma',
    yearBranch: '2nd Year – Data Science',
    ...
    profiles: [],
    skills: [
      { id: 's5', name: 'Python', level: 'Advanced' },
      { id: 's6', name: 'Pandas', level: 'Intermediate' },
    ],
    feedback: [],
  },
};

export default function ReadProfilePage({ readOnly = false, initialData = undefined, noWrapper = false }: { readOnly?: boolean; initialData?: any; noWrapper?: boolean } = {}) {
  const { theme } = useTheme();
  const params = useParams();

  // If rendered as standalone (noWrapper) and an :id is present in the URL, pick sample user data
  const userData = (() => {
    if (initialData) return initialData;
    if (noWrapper && params?.id && sampleUsers[params.id]) return sampleUsers[params.id];
    return undefined;
  })();

  const [avatar, setAvatar] = useState<string | null>(userData?.avatar ?? null);
  const [editingInfo, setEditingInfo] = useState(false);
  const [name, setName] = useState(userData?.name ?? initialData?.name ?? 'Rasika Thakur');
  const [yearBranch, setYearBranch] = useState(userData?.yearBranch ?? initialData?.yearBranch ?? '3rd Year – Computer Science');
  const [email, setEmail] = useState(userData?.email ?? initialData?.email ?? 'rasika@example.com');
  const [phone, setPhone] = useState(userData?.phone ?? initialData?.phone ?? '');

  // Profiles state lifted to the page so they can be visualized elsewhere
  const [profiles, setProfiles] = useState<{ id: string; key: string; url: string }[]>(userData?.profiles ?? initialData?.profiles ?? []);
  ...
  const [bio, setBio] = useState(userData?.bio ?? initialData?.bio ?? 'I am passionate about Java, Python, and Web Development. I enjoy mentoring juniors and learning collaboratively.');
  const [editingBio, setEditingBio] = useState(false);

  const [skills, setSkills] = useState<Skill[]>(userData?.skills ?? initialData?.skills ?? [
  ...
  const [feedback, setFeedback] = useState<Feedback[]>(userData?.feedback ?? initialData?.feedback ?? [
  ...
  // Core main content (without outer wrapper/sidebar)
  const core = (
    <main className="w-full max-w-4xl p-6 md:p-10">
      ...
      <section className="mt-8">
        ...
        <ProfilesWidget profiles={profiles} onAdd={handleAddProfile} onRemove={handleRemoveProfile} readOnly={readOnly || noWrapper} />
      </section>
      ...
      {/* Add Skill Modal */}
      {showAddSkill && !readOnly && !noWrapper && (
  ...
  // If noWrapper is true, render core without the purple bordered container or sidebar
  if (noWrapper) {
    return (
      <div className="min-h-screen w-full flex items-start justify-center p-6 bg-transparent">
        <div className="w-full max-w-4xl">{core}</div>
      </div>
    );
  }

  // Default rendering (keeps existing purple bordered container and sidebar)
  return (
