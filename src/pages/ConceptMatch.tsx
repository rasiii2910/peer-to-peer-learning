  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        const grad = ctx.createRadialGradient(dot.x, dot.y, 0, dot.x, dot.y, dot.size * 4);
        grad.addColorStop(0, 'rgba(59,130,246,0.8)');
        grad.addColorStop(0.4, 'rgba(59,130,246,0.3)');
        grad.addColorStop(1, 'rgba(59,130,246,0)');
...
export default function ConceptMatch() {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full flex flex-col md:flex-row relative bg-neutral-50 dark:bg-neutral-900"
    >
      {/* Space background covers entire container */}
      <SpaceBackground containerRef={containerRef} />
        ...
        <div className="w-full md:w-1/2 h-48 md:h-full relative flex items-center justify-center bg-black/10 p-4">
          ...
          >
            <svg width="80" height="100" viewBox="0 0 80 100" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M40 10 L60 70 L45 80 L40 70 L35 80 L20 70 L40 10 Z" fill="#e5e7eb" stroke="#9ca3af" strokeWidth="1.5" />
              <path d="M40 10 L50 35 L30 35 L40 10 Z" fill="#3b82f6" />
              <circle cx="40" cy="25" r="5" fill="#1e40af" />
              <path d="M20 70 L10 85 L20 80 Z" fill="#dc2626" />
              <path d="M60 70 L70 85 L60 80 Z" fill="#dc2626" />
            </svg>
          </div>
        </div>

        {/* Right column: question and options */}
        <div className="w-full md:w-1/2 flex flex-col p-4 md:p-6 lg:p-8 min-h-0">
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 pb-2">
                {questions[index].options.map((opt, i) => {
                  const isSelected = selected === i;
                  const isCorrect = i === questions[index].correct;
                  const showResult = selected !== null;
                  ...
                  return (
                    <button
                      ...
                      onClick={() => choose(i)}
                      disabled={selected !== null}
                      className={`text-left p-3 md:p-4 rounded-xl border-2 transition-all duration-200 font-medium flex items-start gap-3 min-h-16 md:min-h-20 ${showResult
                        ? (isCorrect
                          ...
                          : isSelected
                            ...
                            : 'border-neutral-200/50 bg-white/50 dark:bg-neutral-800/50 text-neutral-900 dark:text-neutral-300'
                        )
                        : 'border-neutral-200/30 bg-white/70 dark:bg-neutral-800/70 hover:bg-white/90 hover:dark:bg-neutral-700/70 cursor-pointer'
                        }`}
                    >
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white/20 dark:bg-black/20 flex items-center justify-center font-semibold text-xs md:text-sm flex-shrink-0">
                        {String.fromCharCode(65 + i)}
                      </div>
                      <div className="flex-1 text-xs md:text-sm leading-tight">{opt}</div>
                    </button>
