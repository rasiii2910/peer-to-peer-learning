...
export function DebuggingRaceRun() {
  ...
  const [cause, setCause] = useState('Logic Error');
  const intervalRef = useRef<number | null>(null);

  // Sample snippet (lines numbered for display). Bug is on line 4 (0-based index 3) - missing return in function
...
console.log('Result:', result)`;
  ...
  const correctBugLine = 3; // 1-based line number the user should enter (line 4?) We'll validate as 4
  const correctLineNumber = 3; // zero-based index of buggy line: here line 3 (the function is missing return on line 1 but we place bug at line 1)
        ...
        <div className="mt-4">
          ...
          {allowedSubmit && !submitted && (
            <div className="mt-3 flex flex-col md:flex-row gap-3 items-start md:items-center">
              ...
              <label className="w-48">
                ...
                <select value={cause} onChange={(e) => setCause(e.target.value)} className="w-full p-2 border rounded">
                  <option>Logic Error</option>
                  <option>Syntax Error</option>
                  <option>Typo</option>
                  <option>Off-by-one</option>
                  <option>Missing Return</option>
                </select>
              </label>
          ...
          {submitted && (
            <div className="mt-4 p-4 bg-white/60 rounded text-sm text-neutral-800 dark:text-neutral-100">
              ...
              <div className="mt-2 font-semibold">{isCorrect ? 'Correct — well done!' : 'Incorrect — review the snippet again'}</div>
